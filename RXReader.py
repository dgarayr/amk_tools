...
def product_collapser(G):
	'''Contract all equivalent products in the graph to a single node
	Input:
	- G. NetworkX.Graph() as generated by RX_builder() -> connectivity, energy, geometry & frequencies.
	Output:
	...
	for formula,prod_list in formula_map.items():
		if (len(prod_list) == 1):
			continue
		parent = prod_list[0]
		[nx.contracted_nodes(Gwork,parent,child,copy=False) for child in prod_list[1:]]
		...
		collapsed_dict.update({prod:label for prod in prod_list})
...
def poly_path_finder(G,source_collection,target_collection,cutoff,skip_int_frags=True):
	...
	'''
	all_paths = []
	# Get all combinations, via the caresian product implemented in itertools
...
def path_filter(G,path_list,threshold):
	'''For a given list of paths, filter out all paths where a species is above a given energy threshold
	Input:
	- G. NetworkX.Graph() as generated by RX_builder() -> connectivity, energy, geometry & frequencies.
	- path_list. List of lists, containing the fetched paths from single_path_finder() or poly_path_finder()
