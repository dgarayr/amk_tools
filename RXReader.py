...
# Alternative profile generation
...
def theor_profile_builder(G,work_nodes,count,track_dict,edge_bookkeep):
	'''
...
def theor_profile_iterator(G,start_node,print_flag=False):
    '''Iterative generation of profiles via theor_profile_builder(), starting
    from a given node.
    Input:
	- G. NetworkX.Graph() as generated by RX_builder() -> connectivity, energy, geometry & frequencies.
    ...
    iterate_flag = True
    while (iterate_flag):
        next_neighbors,ready_profiles = theor_profile_builder(G,work_nodes,ct,track_dict,edge_bookkeep)
