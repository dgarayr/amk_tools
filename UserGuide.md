...
```python
    ...
    workfolder = "FINAL_LL_MOL"
    rxnfile = "RXNet.cg"
    # The text file is first read into a list of tags and indices
    data = arx.RX_parser(workfolder,rxnfile)
    ...
    G = arx.RX_builder(workfolder,data)
```
...
``` python
    ...
    arx.vibr_displ_parser(workfolder,G)           # locate ALL normal modes and add them to the graph
