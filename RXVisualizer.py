...
def full_view_layout(bokeh_figure,bokeh_graph):
	...
	def load_vibration():
		...
		if (is_node and not is_edge):
			...
			return None
		ndx = sel_source.selected.indices[0]
		# Add a try-except block for cases where frequencies have not been read or for stductures that did not include freqs (such as PROD)
		try:
	...
	# Set up the callback for nodes and edges, which calls load_mol() under the hood whenever a selection (node/edge) is changed
